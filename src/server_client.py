...
# Optional: If you have pybullet installed, uncomment or place behind a try-except
# try:
...
#########################
def start_3d_visualization_client(host="localhost",port=12346, urdf_path='/home/mz/Study/ProjWork/spot-sdk/files/spot_with_arm.urdf'):
   return start_3d_visualization_clients_genesis((host, port), urdf_path)
...
#########################
# 3D Visualization Client (Genesis)
#########################

def start_3d_visualization_clients_genesis(server, urdf_paths):
    """
    A client that connects to multiple servers, receives joint data, and displays multiple 3D robot visualizations
    using PyBullet. Each URDF file must be specified for each server connection.
    """
    ...
    # Use URDF file to load Spot robot
    spot = scene.add_entity(
        gs.morphs.URDF(
            file = urdf_paths,
        ),
    )
    # print all available joint names for confirmation of correct names
    all_joint_names = [joint.name for joint in spot.joints]
    print("Available joints:", all_joint_names)
                    ...
                    while b'\n' in buffers[str(server)]:
                        ...
                        if line:
                            try:
                                ...
                                # get the indices of freedom in robot's joint 
                                # dofs_idx = [spot.get_joint(name).dof_idx_local for name in joint_names]
                                ...
                                for joint_name, angle in data.items():
                                    ...
                                    if joint_name in all_joint_names:
                                        dofs_id = spot.get_joint(joint_name).dof_idx_local
                                        target_position.append(angle)
                                        dofs_idx.append(dofs_id)
                                   
                                print(target_position)
                                spot.set_dofs_position(
                                            target_position,
                                            dofs_idx,
                                        )
                                spot.set_pos(pos)
